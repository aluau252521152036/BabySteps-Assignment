...
app.get("/doctors/:id/slots", async (req, res) => {
  try {
    ...
    const bookedSlots = appointments.map(app => format(app.date, "yyyy-MM-dd HH:mm"));
    const availableSlots = possibleSlots.filter(slot => !bookedSlots.includes(format(slot, "yyyy-MM-dd HH:mm")));
